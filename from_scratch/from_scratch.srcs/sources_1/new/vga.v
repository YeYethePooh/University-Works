module vga(
    ...
    input btnC,       // Button to write character or newline
    output hsync,
    output vsync,
    output [11:0] rgb
);
    ...
    always @(posedge p_tick or posedge reset) begin
        if (reset) begin
            rgb_reg <= 12'h031; // Default color on reset
            ...
            // Character Rendering Logic
                ...
                if (rom_data[8 - x_in_char_col])  // 8 - x because bit to use for x = 0 is the most sig bit 8'b10000000;
                    ...
                    rgb_reg <= 12'h005; // Dark Blue for background
            end 
            // Background Logic
            else begin
