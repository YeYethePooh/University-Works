...
//               .dte(btnU), .clk(clk));
    ...
    // Display
    quadSevenSeg seven_segment_display (
        .seg(seg),
        .dp(dp),
        .an(an),
        .num3(data_in[7:4]),  // High nibble of received data
        .num2(data_in[3:0]),  // Low nibble of received data
        .num1(data_in[7:4]),  // High nibble of transmitted data
        .num0(data_in[3:0]),  // Low nibble of transmitted data
        .clk(clkDiv)
    );
    // Reset
    always @(reset) begin
